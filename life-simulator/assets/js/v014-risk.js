(() => {
  ...
  function event(icon, title, story, choices) { return { id: typeof createId === "function" ? createId() : String(Date.now()), icon, title, story, choices }; }
  function economicShock() {
    const career = state.career;
    const business = career?.path === "business";
    return event("📉", business ? "市场突然冷下来" : "公司开始收缩预算", business ? `${career.name}连续几周询价变多、成交变少。客户不是不需要，只是每一笔钱都批得更慢。` : `${career?.employer ?? "公司"}通知下个季度控制成本，绩效奖金和招聘计划都被重新评估。`, [
  ...
  function healthCrisis() {
    return event("🚑", "身体终于发出硬警告", "连续疲惫之后，你在一个普通早晨明显撑不住了。问题不一定严重，但已经不是睡一觉就能假装过去。", [
  ...
  function contractRisk() {
    ...
    return event("⚖️", business ? "一份模糊合同开始反咬" : "口头安排变成了责任争议", business ? "客户拿出聊天记录要求你按最初口头承诺补交额外内容，但报价里从没写清这部分边界。" : "一项临时任务出了问题，大家对谁答应过什么说法不一，你发现很多安排只停留在群聊里。", [
  ...
  function livingCostShock() {
    return event("🏘️", "房东通知下季度涨租", `你现在住在${state.career.housing}。房东说周边租金都涨了，下季度要重新定价。`, [
  ...
  function updateRisks(action) {
    ...
    let scene = null;
    if (state.career && risk.economyHeat <= 28 && Math.random() < 0.32) scene = economicShock();
    else if (["dropout", "work"].includes(state.education.track) && (state.stats.health <= 24 || risk.medicalDebt >= 12) && Math.random() < 0.45) scene = healthCrisis();
    else if (state.career && risk.legalRisk >= 55 && Math.random() < 0.34) scene = contractRisk();
    else if (state.career?.location === "away" && Math.random() < 0.08) scene = livingCostShock();
    if (scene) risk.lastShockTurn = state.turn;
    return scene ? [scene] : [];
  }
  const oldRenderGame = renderGame;
  renderGame = function patchedRenderGame() { oldRenderGame(); ensureRisks(); injectRiskTag(); };
  const oldTakeAction = takeAction;
  takeAction = function patchedTakeAction(action) { oldTakeAction(action); if (state?.pendingScenes) state.pendingScenes.push(...updateRisks(action)); saveGame(); injectRiskTag(); };
